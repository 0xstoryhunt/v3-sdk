...
describe('Trade', () => {
  ...
  const pool_wip_0 = v2StylePool(
    CurrencyAmount.fromRawAmount(WIP9[1516], JSBI.BigInt(100000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100000))
  )

  const pool_wip_1 = v2StylePool(
    CurrencyAmount.fromRawAmount(WIP9[1516], JSBI.BigInt(100000)),
    CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(100000))
  )

  const pool_wip_2 = v2StylePool(
    CurrencyAmount.fromRawAmount(WIP9[1516], JSBI.BigInt(100000)),
    CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100000))
  )

  describe('#fromRoute', () => {
    it('can be constructed with IP as input', async () => {
      const trade = await Trade.fromRoute(
        new Route([pool_wip_0], _IP, token0),
        CurrencyAmount.fromRawAmount(IP.onChain(1516), JSBI.BigInt(10000)),
        TradeType.EXACT_INPUT
      )
      expect(trade.inputAmount.currency).toEqual(_IP)
      expect(trade.outputAmount.currency).toEqual(token0)
    })
    it('can be constructed with IP as input for exact output', async () => {
      const trade = await Trade.fromRoute(
        new Route([pool_wip_0], _IP, token0),
    ...
    it('can be constructed with _IP as output', async () => {
      const trade = await Trade.fromRoute(
        new Route([pool_wip_0], token0, _IP),
        CurrencyAmount.fromRawAmount(IP.onChain(1516), JSBI.BigInt(10000)),
        TradeType.EXACT_OUTPUT
      )
      expect(trade.inputAmount.currency).toEqual(token0)
      expect(trade.outputAmount.currency).toEqual(_IP)
    })
    it('can be constructed with _IP as output for exact input', async () => {
      const trade = await Trade.fromRoute(
        new Route([pool_wip_0], token0, _IP),
        CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10000)),
        TradeType.EXACT_INPUT
  ...
  describe('#fromRoutes', () => {
    it('can be constructed with _IP as input with multiple routes', async () => {
      const trade = await Trade.fromRoutes<IP, Token, TradeType.EXACT_INPUT>(
        [
          {
            amount: CurrencyAmount.fromRawAmount(IP.onChain(1516), JSBI.BigInt(10000)),
            route: new Route([pool_wip_0], _IP, token0)
          }
        ],
        TradeType.EXACT_INPUT
      )
    ...
    it('can be constructed with _IP as input for exact output with multiple routes', async () => {
      const trade = await Trade.fromRoutes<IP, Token, TradeType.EXACT_OUTPUT>(
        [
          {
          ...
          },
          {
            ...
            route: new Route([pool_wip_1, pool_0_1], _IP, token0)
          }
        ],
        TradeType.EXACT_OUTPUT
      )
    ...
    it('can be constructed with _IP as output with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Token, IP, TradeType.EXACT_OUTPUT>(
        [
          {
            amount: CurrencyAmount.fromRawAmount(IP.onChain(1516), JSBI.BigInt(4000)),
            route: new Route([pool_wip_0], token0, _IP)
          },
          {
            amount: CurrencyAmount.fromRawAmount(IP.onChain(1516), JSBI.BigInt(6000)),
            route: new Route([pool_0_1, pool_wip_1], token0, _IP)
          }
        ],
        TradeType.EXACT_OUTPUT
      )
    ...
    it('can be constructed with _IP as output for exact input with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Token, IP, TradeType.EXACT_INPUT>(
        [
          {
            amount: CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(3000)),
            route: new Route([pool_wip_0], token0, _IP)
    ...
    it('throws if pools are re-used between routes', async () => {
      await expect(
        Trade.fromRoutes<Token, IP, TradeType.EXACT_INPUT>(
          [
            {
            ...
            },
            {
              ...
              route: new Route([pool_0_1, pool_1_2, pool_wip_2], token0, _IP)
            }
          ],
          TradeType.EXACT_INPUT
        )
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for _IP currency input', async () => {
      const result = await Trade.bestTradeExactIn(
        [pool_wip_0, pool_0_1, pool_0_3, pool_1_3],
        CurrencyAmount.fromRawAmount(IP.onChain(1516), JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(_IP)
      expect(result[0].swaps[0].route.tokenPath).toEqual([WIP9[1516], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(_IP)
      expect(result[1].swaps[0].route.tokenPath).toEqual([WIP9[1516], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })

    it('works for _IP currency output', async () => {
      const result = await Trade.bestTradeExactIn(
        [pool_wip_0, pool_0_1, pool_0_3, pool_1_3],
      ...
      expect(result[0].swaps[0].route.tokenPath).toEqual([token3, token0, WIP9[1516]])
      expect(result[0].outputAmount.currency).toEqual(_IP)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].swaps[0].route.tokenPath).toEqual([token3, token1, token0, WIP9[1516]])
      expect(result[1].outputAmount.currency).toEqual(_IP)
    })
  })
